...
if !isdir("resultats/tmp")
  mkdir("resultats/tmp")
end

createpvd("resultats/results") do pvd
  pvd[0] = createvtk(Ω, "resultats/tmp/results_0" * ".vtu", cellfields=["u" => uh0],
        compress = false,
        append   = false)
  for (tn, uhn) in uh
    pvd[tn] = createvtk(Ω, "resultats/tmp/results_$tn" * ".vtu", cellfields=["u" => uhn],
        compress = false,
        append   = false)
  end
end